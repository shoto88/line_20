    ...
    summaryData.forEach((ticket) => {
      ...
      if (ticketDate >= twelveWeeksAgo) {
        ...
        const weekIndex = weeklyData.findIndex(
          (w) => w.weekStart === format(weekStart, "yyyy-MM-dd")
        );
        if (weekIndex !== -1) {
      ...
      if (
        ...
        const dayIndex = dailyData.findIndex(
          (d) => d.date === format(ticketDate, "M/d")
        );
        if (dayIndex !== -1) {
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
        <Card className="mt-16">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={processedData.todayHourlyData.map((count, index) => ({
                  hour: index + 10,
                  count,
                }))}
              >
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-sm text-gray-500">
              {format(subWeeks(new Date(), 11), "yyyy年MM月dd日", {
                locale: ja,
              })}{" "}
              から
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={processedData.weeklyData}>
                ...
                <XAxis
                  dataKey="weekStart"
                  tickFormatter={(tick) =>
                    format(parseISO(tick), "M/d", { locale: ja })
                  }
                />
                <YAxis />
                <Tooltip
                  labelFormatter={(label) =>
                    `週開始: ${format(parseISO(label), "yyyy/MM/dd", { locale: ja })}`
                  }
                />
                <Legend />
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={processedData.hourlyData.map((count, index) => ({
                  hour: index + 10,
                  count,
                }))}
              >

...
const UserInfo = () => {
  ...
  const notifyMutation = useMutation({
    mutationFn: (ticketNumber: number) => 
      axios.post(`${import.meta.env.VITE_API_URL}/api/notify-user/${ticketNumber}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['d1'] });
    },
  });
  ...
  const columns = [
    ...
    columnHelper.accessor("examination_number", {
      ...
      cell: (info) => {
        const ticket = info.row.original;
        const [inputValue, setInputValue] = React.useState<string>(ticket.examination_number?.toString() ?? "");

        const { mutate, isPending } = useMutation({
          mutationFn: async ({ examinationNumber }: { examinationNumber: number }) => {
            await axios.put(`${import.meta.env.VITE_API_URL}/api/follow/${ticket.ticket_number}/examination-number`, { examinationNumber });
          },
          onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['d1'] });
          },
        ...
        return (
          <div className="flex items-center space-x-2">
            ...
            }} disabled={isPending} className='w-10 bg-blue-300 h-6 text-white'>
              {isPending ? '保存中...' : '登録'}
            </Button>
          </div>
        );
    ...
    columnHelper.accessor("notified", {
      ...
      cell: (info) => {
        ...
        return (
          <Button 
            onClick={() => notifyMutation.mutate(ticket.ticket_number)}
            disabled={notifyMutation.isPending || ticket.notified}
            className={`w-20 h-6 text-white ${ticket.notified ? 'bg-gray-300' : 'bg-blue-500 hover:bg-blue-600'}`}
          >
            {ticket.notified ? '通知済み' : (notifyMutation.isPending ? '送信中...' : '通知する')}
          </Button>
        );
      },
    }),

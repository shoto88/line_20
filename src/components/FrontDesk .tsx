...
const UserInfoWithoutPatientQueue = () => {
  ...
  const { data, status, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        const apiUrl = `${import.meta.env.VITE_API_URL}/api/lineinfo`;
    ...
    return (
      <div className="text-red-500 p-4">
        ...
        {axiosError?.response?.data && (
          <div className="mt-2">
            詳細: {JSON.stringify(axiosError.response.data)}
          </div>
        )}
      </div>
    );
  ...
  const lineIssuedNumbers = data
    ? data.map((ticket: Ticket) => ticket.ticket_number)
    : [];
  useEffect(() => {
      ...
      const newTickets = data.filter(
        (ticket: Ticket) =>
          !prevDataRef.current.some(
            (prevTicket: Ticket) =>
              prevTicket.ticket_number === ticket.ticket_number
          )
      );
        ...
        >
          ...
          <DialogFooter className="m-4">
            <Button
              ...
              className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded-md text-lg transition-colors duration-200"
            >
              OK
            </Button>
          </DialogFooter>
